	...
	$.PullToRefresh = $.Class.extend({
		init: function(element, options) {
			...
			this.options = $.extend(true, {
				...
				up: {
					auto: false,
					offset: 70, //距离底部高度(到达该高度即触发)
					show: true,
					contentdown: '上拉显示更多',
					contentrefresh: '正在加载...',
					contentnomore: '--没有更多数据了--',
					contentnodata:'<div style="width:70%;margin:60px auto;text-align:center;font-size:18px;"><p style="width:120px;margin:0 auto 2px;"><img src="http://7xoq3p.com1.z0.glb.clouddn.com/nodata-2.png" style="width:100%;"></p>目前木有数据</div>',
					callback: false
				},
		...
		endPullUpToRefresh: function(finished) {
			if (finished) {
				this.finished = true;
				if(finished=="nodata"){
					this.pullUpTipsIcon.innerHTML = this.options.up.contentnodata;
				}else if(finished=="nomore"){
					this.pullUpTipsIcon.innerHTML = '';
				}else {
