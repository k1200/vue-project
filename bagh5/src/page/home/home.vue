<template>
  <article>
    <div class="mui-content" id="home">
      <section class='head'>
        <img :src="appset.toppic" style='width:100%;' />
      </section>
      <section class="goods">
        <ul>
          <router-link tag="li" class="mui-card goods_item" v-for="(item) in activities" :to="'../bag/bag.html?id=' + item.id" :key="item.id" :data-state="item.mark">
            <div class="mui-card-content">
              <img class='goods_item_image' :src='item.pic_1' width="100%" />
              <div class='down_time'>
                {{ item.page_timestamp}}
              </div>
            </div>
            <div class="mui-card-footer">
              ...
              <div v-if="item.change.length>0" class='goods_item_user'>
                <img v-for="(list) in item.change" :key = "list.unionid" :src="list.headimg" />
              </div>
            </div>
          </router-link>
        </ul>
